<template>
  ...
  <div v-else class="pdv-main">
      <div class="pdv-top">
        <section class="card form-card">
          <div class="card-header">
            ...
            <div class="card-actions">
              <button v-if="editingId" class="btn btn-outline btn-sm" style="color:#ef4444;border-color:rgba(239,68,68,0.3)" @click="resetForm">
        ...
        <section class="card">
          <div class="card-header">
            <svg viewBox="0 0 24 24" class="card-icon" style="color:#f59e0b" v-once fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>
            <div>
          ...
          <div class="pdv-h-list" v-if="holidays.length">
            <div class="pdv-h-row" v-for="h in holidays" :key="h.date">
              ...
              <button class="btn btn-ghost btn-xs" style="color:#ef4444" @click="delHoliday(h.date)">
      ...
      <section class="card">
        <div class="card-header">
          ...
          <div>
            ...
            <p class="card-desc">共 {{ records.length }} 条，合计 {{ totalPersonDays }} 人天</p>
          </div>
          <div class="card-actions">
            ...
            <tbody>
              <tr v-for="r in records" :key="r.id">
                ...
                <td class="col-act">
                  ...
                  <button class="btn btn-ghost btn-xs" style="color:#ef4444" @click="delRecord(r.id)">
                    <svg viewBox="0 0 24 24" class="btn-icon" style="width:13px;height:13px" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>
                    删除
                  </button>
                </td>
...
<style scoped>
.pdv-main { max-width: 1200px; margin: 0 auto; display: flex; flex-direction: column; gap: 12px; }
...
.form-body { flex: 1; display: flex; flex-direction: column; gap: 12px; }
.pdv-form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px 12px; }
.field-row { display: flex; flex-direction: column; gap: 4px; }
.field-row-span { grid-column: 1 / -1; }
.field-label { font-size: 12px; color: var(--muted-foreground); font-weight: 500; margin-bottom: 2px; }
.field-input {
...
.field-dt { font-family: 'JetBrains Mono','Consolas',monospace; font-size: 13px; letter-spacing: 0.3px; color-scheme: dark; }
.form-actions { display: flex; gap: 8px; justify-content: center; margin-top: auto; padding: 16px 0 8px; }

.btn-block-wide { width: 100%; justify-content: center; }

.pdv-sync-msg { font-size: 12px; color: #22c55e; text-align: center; margin-bottom: 10px; white-space: pre-line; word-break: break-all; }
.pdv-h-add { display: flex; gap: 8px; margin-bottom: 12px; align-items: center; }
.pdv-h-add .field-input { flex: 1; min-width: 0; }
.pdv-h-add .field-input[type="date"] { font-family: 'JetBrains Mono','Consolas',monospace; font-size: 13px; }
.pdv-h-list { max-height: 220px; overflow-y: auto; background: rgba(17,17,19,0.2); border-radius: var(--radius); border: 1px solid var(--border); }
.pdv-h-row { display: flex; align-items: center; gap: 10px; padding: 7px 10px; border-bottom: 1px solid rgba(39,39,42,0.3); font-size: 13px; }
.pdv-h-row:last-child { border-bottom: none; }
.pdv-h-date { color: var(--destructive); font-weight: 500; font-family: 'Consolas', monospace; font-size: 12.5px; flex-shrink: 0; }
.pdv-h-name { color: var(--muted-foreground); flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

.pdv-spin { display: inline-block; width: 14px; height: 14px; border: 2px solid rgba(59,130,246,0.25); border-top-color: var(--primary); border-radius: 50%; animation: spin 0.6s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }

.pdv-wrap { max-height: 520px; overflow-y: auto; border: 1px solid var(--border); border-radius: var(--radius); }
...
.pdv-tbl th {
  padding: 10px 8px; text-align: center; font-weight: 600; font-size: 10.5px;
  color: var(--muted-foreground); text-transform: uppercase; letter-spacing: 0.5px;
  background: rgba(24,24,27,0.6); border-bottom: 1px solid var(--border);
}
.pdv-tbl td { padding: 9px 8px; text-align: center; border-bottom: 1px solid rgba(39,39,42,0.3); color: var(--muted-foreground); }
.pdv-tbl tbody tr { background: rgba(17,17,19,0.25); transition: background .15s; }
.pdv-tbl tbody tr:hover { background: rgba(59,130,246,0.06); }
.pdv-tbl tbody tr:last-child td { border-bottom: none; }

.col-name { width: 12%; text-align: left !important; color: var(--foreground) !important; font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.col-work { width: 17%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.col-time { width: 12%; font-family: 'JetBrains Mono','Consolas',monospace; font-size: 11.5px; letter-spacing: 0.2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.col-num { width: 7%; }
.col-act { width: 12%; }
td.col-num { font-variant-numeric: tabular-nums; }
td.days { color: #22c55e !important; font-weight: 600; font-size: 13.5px; }
td.ot { color: #f59e0b; }
td.hl { color: var(--destructive); }
</style>
