  ...
  <div class="benchmarks-view">
    <div class="bm-toolbar">
      <div class="bm-search-wrap">
        ...
        <input type="text" class="bm-search-input" v-model="searchText" placeholder="搜索巡检项名称或说明..." />
      </div>
      <div class="bm-tag-group">
        <button class="bm-tag" :class="{ active: activeCategory === '' }" @click="activeCategory = ''; page = 1">全部</button>
      ...
      <span class="bm-count">{{ filteredItems.length }} 条结果</span>
    </div>
    ...
    <div v-else class="bm-table-wrap">
      <table class="bm-table">
        <thead>
          <tr>
            <th class="col-name">巡检项名称</th>
            <th class="col-cat">分类</th>
            <th class="col-baseline">基准值/阈值</th>
            <th class="col-ok"><span class="badge ok">✓ 正常</span></th>
            <th class="col- warn"><span class="badge warn">⚠ 注意</span></th>
            <th class="col-err"><span class="badge err">✗ 异常</span></th>
        ...
        <tbody>
          <tr v-for="(row, i) in pagedItems" :key="i">
            ...
            <td class="col-cat"><span class="cat-chip" :style="{ borderColor: catColors[row.category] || '#30363d', color: catColors[row.category] || '#8b949e' }">{{ row.category }}</span></td>
    ...
    <div class="bm-pager" v-if="totalPages > 1">
      <button class="btn btn-outline btn-sm" :disabled="page <= 1" @click="page--">
        <svg viewBox="0 0 24 24" class="btn-icon" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 18l-6-6 6-6"/></svg>
        上一页
      </button>
      <span class="bm-page-info">第 {{ page }} / {{ totalPages }} 页，共 {{ filteredItems.length }} 条</span>
      <button class="btn btn-outline btn-sm" :disabled="page >= totalPages" @click="page++">
        下一页
        <svg viewBox="0 0 24 24" class="btn-icon" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18l6-6-6-6"/></svg>
      </button>
    </div>
  </div>
</template>
...
.bm-toolbar { display: flex; align-items: center; gap: 14px; margin-bottom: 16px; flex-wrap: wrap; }

.bm-search-wrap { position: relative; flex: 0 0 280px; }

.bm-search-icon { position: absolute; left: 10px; top: 50%; transform: translateY(-50%); width: 16px; height: 16px; color: var(--muted-foreground); }

.bm-search-input {
  width: 100%; padding: 8px 12px 8px 32px; background: transparent;
  border: 1px solid var(--border); border-radius: var(--radius);
  color: var(--foreground); font-size: 13px; outline: none; transition: border-color 0.2s;
}
.bm-search-input:focus { border-color: var(--primary); }
.bm-search-input::placeholder { color: var(--muted-foreground); }

.bm-tag-group { display: flex; gap: 5px; flex-wrap: wrap; }

.bm-tag {
  padding: 4px 12px; border: 1px solid var(--border); border-radius: 20px;
  background: transparent; color: var(--muted-foreground); font-size: 12px; cursor: pointer;
  transition: all 0.15s; white-space: nowrap;
}
.bm-tag:hover { border-color: var(--border-hover); color: var(--foreground); }
.bm-tag.active { background: rgba(59,130,246,0.12); border-color: var(--primary); color: var(--primary); }

.bm-count { font-size: 12px; color: var(--muted-foreground); margin-left: auto; white-space: nowrap; }

.empty-text { font-size: 14px; color: var(--muted-foreground); }

.bm-table-wrap { overflow-x: auto; border: 1px solid var(--border); border-radius: var(--radius); }

.bm-table { width: 100%; border-collapse: collapse; font-size: 13px; }

.bm-table thead { background: var(--muted); position: sticky; top: 0; z-index: 1; }

.bm-table th {
  padding: 10px 12px; text-align: left; font-weight: 600; color: var(--muted-foreground);
  font-size: 11px; text-transform: uppercase; letter-spacing: 0.4px;
  border-bottom: 1px solid var(--border); white-space: nowrap;
}

.bm-table td {
  padding: 10px 12px; border-bottom: 1px solid var(--border); color: var(--foreground); vertical-align: middle;
}

.bm-table tbody tr { background: rgba(17,17,19,0.25); transition: background 0.1s; }
.bm-table tbody tr:hover { background: rgba(59,130,246,0.05); }
.bm-table tbody tr:last-child td { border-bottom: none; }

.col-name { width: 220px; }
.col-name code { font-size: 12px; color: var(--primary); background: rgba(59,130,246,0.08); padding: 2px 6px; border-radius: 4px; }
.col-cat { width: 86px; white-space: nowrap; }
.col-baseline { min-width: 130px; font-size: 12.5px; }
.col-ok { min-width: 140px; font-size: 12.5px; color: #22c55e; }
.col-warn { min-width: 110px; font-size: 12.5px; color: #f59e0b; }
.col-err { min-width: 120px; font-size: 12.5px; color: var(--destructive); }
.col-desc { min-width: 160px; font-size: 12.5px; color: var(--muted-foreground); line-height: 1.4; }
...
.badge.err { background: rgba(239,68,68,0.1); color: var(--destructive); }
...
.bm-page-btn:hover:not(:disabled) { border-color: var(--primary); color: var(--primary); }
.bm-page-btn:disabled { opacity: 0.3; cursor: not-allowed; }

.bm-page-info { font-size: 13px; color: var(--muted-foreground); }
</style>
